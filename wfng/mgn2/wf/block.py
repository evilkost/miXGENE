...
from .fields import ParamField, InputType, FieldType, BlockField
...
class BlockMeta(abc.ABCMeta):
    def __new__(mcs, name, bases, attrs):
        super_new = super(BlockMeta, mcs).__new__
...
class BaseBlock(object):
    ...
    def create(cls):
        ...
        obj = cls()
        def inject_fields(fields_list, doc):
            for field in cls._init_fields:
                print(field.__dict__)

        inject_fields(obj.doc)
        obj.initiated = True

    @classmethod
    def load(cls, conn, uuid):
        """
        :type conn: pymongo client
        """
        obj = cls()
        obj.doc = conn[cls._COLLECTION_NAME].find_one({"uuid": uuid})

    def save(self, conn):
        conn[self._COLLECTION_NAME].insert_one(self.doc)
